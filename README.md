...
| **S3 Browser (desktop)** | ✅ Desktop app (not web self-hosted) | ✅ S3 & compatible supported | ❌ No team/role server (desktop client) | ✅ Uses user auth locally; can integrate with keys | ✅ Share via signed URLs (depends on backend) | ❌ No background indexed folder sizes server-side | ✅ Presigned and direct S3 operations supported | ✅ Bulk operations supported in client | ⭐⭐⭐ Polished desktop UX | ✅ Desktop install (not docker) | Proprietary (free for personal use) |
